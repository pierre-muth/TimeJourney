...
def light_meas(camera):
    ...
    if light_adc < 6700:
        ...
        camera.iso = 100
        stack_mode_request = True
        camera.shutter_speed = int(3.147e23*light_adc**-4.371)
        
    fd = open('light_meas.csv','a')
    fd.write('%d, %d, %d, %d, %d, %d, %f, %f, %d\n' %
             (shot_min, shot_max, shot_mean, light_adc, camera.exposure_speed, camera.iso, camera.awb_gains[0], camera.awb_gains[1], time.time()))
    fd.close()
